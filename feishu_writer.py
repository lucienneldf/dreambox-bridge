...
class FeishuWriter:
    ...
    AI_FIELD_DEFINITIONS = [
        {"field_name": "是否筛选", "type": 11},  # 复选框
        {"field_name": "是否生成", "type": 11},  # 复选框
        {"field_name": "质量等级", "type": 3, "property": {
            "options": [
                {"name": "S1"},
                {"name": "S2"},
                {"name": "S3"},
        ...
        {"field_name": "判定依据", "type": 1},  # 多行文本
        {"field_name": "注意点", "type": 1},  # 多行文本
        {"field_name": "筛选状态", "type": 3, "property": {
            "options": [
                {"name": "待筛选"},
                {"name": "已完成"},
                {"name": "失败"},
        ...
        {"field_name": "生成文案", "type": 1},  # 多行文本
        {"field_name": "生成状态", "type": 3, "property": {
            "options": [
                {"name": "待生成"},
                {"name": "已完成"},
                {"name": "失败"},
    ...
    def _get_token(self, max_retries=3):
    ...
    def ensure_ai_fields(self):
        """确保 AI 相关字段存在，不存在则创建"""
        existing = self._get_existing_fields()
        created = []
        for field_def in self.AI_FIELD_DEFINITIONS:
            if field_def["field_name"] in existing:
                continue
            resp = requests.post(
                f"{self.BASE_URL}/bitable/v1/apps/{self.app_token}/tables/{self.table_id}/fields",
    ...
    def read_records(self, page_size=100, page_token=None, max_retries=3):
        """读取表格记录（支持分页，带重试机制）"""
